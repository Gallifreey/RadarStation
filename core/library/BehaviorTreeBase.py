...
class BTActionNodeAnnotation(Singleton):
    __actions = []
    ...
    def __check_is_camel_with_node(_str):
        ...
        if (_str.endswith('ActionNode') or _str.endswith('ConditionNode')) and bool(re.match(pattern, _str)) and _str[
            0].isupper():
            return True
        return False
...
# BehaviourTreeGraph().build(r'F:\Python\GlobalRadar\backend\resource\behavior_tree_config\config.json')
